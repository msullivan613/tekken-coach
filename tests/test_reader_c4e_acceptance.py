...
def test_the_written_table_decodes_a_framerecord(derived: OffsetTable) -> None:
    ...
    assert decoded.match_state is expected.match_state
...
def test_the_doctor_passes_on_the_written_table(derived: OffsetTable) -> None:
    ...
    # advance and the players must move, so the source replays successive planted snapshots.
    frames = [component_decode_source(derived, step=i) for i in range(6)]
    g = derived.global_struct
    advance_on = resolve_anchor(frames[0], g.anchor) + g.fields["frame_counter"].offset
    source = TickingFlatSource(frames, advance_on=advance_on)

    report = run_doctor(source, derived, known_char_ids=KNOWN_CHAR_IDS, frames=6)
...
def test_frames_poll_forward_with_no_gaps(derived: OffsetTable) -> None:
    frames = [component_decode_source(derived, step=i) for i in range(4)]
    g = derived.global_struct
    advance_on = resolve_anchor(frames[0], g.anchor) + g.fields["frame_counter"].offset
    reads = poll_frames(TickingFlatSource(frames, advance_on=advance_on), derived, 4)
